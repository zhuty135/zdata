#!/usr/bin/python3
import sys
sys.path.append('/work/tma/project/gitrepos/finger/zlib/')
from zutils import get_prev_business_date 

import tushare as ts
mytoken = 'dfb6e9f4f9a3db86c59a3a0f680a9bdc46ed1b5adbf1e354c7faa761'
...
def check_and_delete_db_record(s,mdb_str,del_flag=False):
    db=sqlite3.connect('/tmp/checkdelete.db')#mongo_client[mdb_str]#admin
    if del_flag:
        try:
...
def basic_to_db(dk,ex,d_type, df, oflag, verbose=True):
    if verbose:
        estr = '/work/tma/db/basic/' + dk + '_' + ex + '.db'#mongo_client[mdbstr]
        ...
        if check:
            ...
            doclist = doc['ts_code'] #for doc in mco.find()]
            if verbose:  df.to_csv('/tmp/udf.'+dk+'.'+ex)
            udf = df[~df['ts_code'].isin(doclist)]#str.contains
        else:
            udf = df
        if udf.empty:
...
def remove_duplicates(coll):
    ...
    return result#mycollection.aggregate(pipeline) 

#fs_list =  ['fina_indicator','income','balancesheet','cashflow','dividend')# 'daily_fina_indicator','daily_income']
fs_list =  ['dividend']# 'daily_fina_indicator','daily_income')
def write_to_db(i,df, ded, fflag, oflag, cdict,keystr='date',verbose=True):
    ...
    if oflag:
        try:
            if fflag:#d_type == 'fullhist' or d_type in ('fina_indicator','income'):
                print('FULL HISTORY drop then insert')
                df.to_sql(name=i, con=ded, if_exists='replace',index=False)#,index_label='date')
            else:
                df.to_sql(name=i, con=ded, if_exists='append',index=False)#,index_label='date')
        except Exception as e:
            print(e)
...
def fetch_fs_data(i,f,s,e,dk):
    fcallbasic = 'pro.' + f + "(ts_code='"+ i + "',start_date='" + s + "',end_date='" + e #+ "',fields=" + fs_fld + ")" 
    fcall = fcallbasic  + "')"
    print(fcall)
    df = eval(fcall) 
    if df is None or df.empty:
...
def fetch_daily_data(i,s,e,dk):
    ...
    time.sleep(1)
    if dk in ('fut','opt'): 
        #20190630 flds = ["trade_date", "open", "high", "low", "close", "vol",'oi','amount','settle']
        df["adjusted"] = df['settle']
    else:
        #20190630 flds = ["trade_date", "open", "high", "low", "close", "vol",'amount']
        df["adjusted"] = df["close"]
    return df

def bar_to_db(dk,ex,d_type,sd,ed,fflag,oflag,verbose=True):
    #shortname =  dk + '_'+d_type+'_' + ex + '.db' if d_type in ('fina_indicator','income')  else  dk + '_' + ex + '.db'
    estr = '/work/tma/db/basic/'  + dk + '_' + ex + '.db'#mdb = mongo_client[ dk + '_' + ex ]
    deb = create_engine('sqlite:///' + estr)
    #shortname =  dk + '_'+d_type+'_' + ex + '.db' if d_type in fs_list  else  dk + '_' + ex + '.db'
    shortname =  dk + '_' + ex + '.db'
    dailystr = '/work/tma/db/' + d_type + '/'+ shortname
    ded = create_engine('sqlite:///' + dailystr)# mdb_daily = mongo_client[ dailystr ]

    print(estr,dailystr)
    symbols = ded.table_names()#mdb_daily.collection_names()
    print(symbols)
    ...
    if verbose:
        if True:#d_type in ('fullhist','daily') or d_type in fs_list:
            for i in  bdf.index:
                cdict = {}
                ...
                if dk in ('fund_nav','fund'): 
                    if d_type=='daily':
                        ...
                        #fcall = fcallbasic + ",adj='hfq')" if dk == 'stock' eles fcallbasic +"')"  
                        time.sleep(1)
                    ...
                    #20190630 flds = ['end_date','open','high','low','close','volume','adjusted']
...
def get_tu_data(d_path,sd,ed,dk = 'opt',d_type='basic',fflag=False,oflag=False,verbose=True):
    ...
    for k,ex in zdict.items():
        ...
        else:#if d_type in ('fullhist','daily') or d_type in fs_list:
...
def main():
    ...
    if test_code is not None:
        mfs = test_code.split('.')#'M1909-C-2850.DCE'#'A2003.DCE',
        mdb_str = dkey + '_' + mfs[1].lower() + '_daily'# 'fut_dce_daily'
        print(mdb_str)
    ...
    if dkey in ('opt','fut','fund_nav','index','stock'):
        if fullhist_flag:
            ...
            if dkey in ('stock',):
                #get_tu_data(input_path,sdate,edate,dk=dkey, d_type='fina_indicator',fflag=fullhist_flag,oflag=output_flag)
                #get_tu_data(input_path,sdate,edate,dk=dkey, d_type='income',fflag=fullhist_flag,oflag=output_flag)
                #get_tu_data(input_path,sdate,edate,dk=dkey, d_type='balancesheet',fflag=fullhist_flag,oflag=output_flag)
                #get_tu_data(input_path,sdate,edate,dk=dkey, d_type='cashflow',fflag=fullhist_flag,oflag=output_flag)
                for k in fs_list:
                    print('k',k)
            ...
            sdate = get_prev_business_date(date.today(), n)# sdate = get_prev_business_date(date.today() - timedelta(0), n)#.strftime("%Y%m%d")
